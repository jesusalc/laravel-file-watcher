...
class WatchFileSystem extends Command
{
	...
	{
		$pid = getmypid();
		$bootId = bin2hex(random_bytes(3));
		Log::channel('watcher')->info("BOOT [$bootId] PID [$pid] START");
        $handlers = Log::channel('watcher')->getLogger()->getHandlers();
        Log::channel('watcher')->info("Watcher has " . count($handlers) . " handler(s)");

		if (!function_exists('inotify_init')) {
			$this->error("inotify extension not installed.");
			return Command::FAILURE;
		}
        Log::channel('watcher')->warning("Calling logStartup() from handle()");
        $this->logStartup($dispatcher);
		...
		while (true) {
			...
			if ($events) {
                ...
                foreach ($events as $event) {
                    ...
                    if ($mask & IN_CREATE) {
                        HandleFileEvent::dispatch('create', $filePath);
                    }

                    if ($mask & IN_MODIFY) {
                        HandleFileEvent::dispatch('modify', $filePath);
                    }

                    if ($mask & IN_DELETE) {
                        HandleFileEvent::dispatch('delete', $filePath);
                    }
                }
				// foreach ($events as $event) {
				// 	$filePath = $watchPath . '/' . $event['name'];
	...
	protected function logStartup(DispatcherService $dispatcher): void
	{
        ...
        Log::channel('watcher')->error("STARTUP: Logger initialized");
        Log::channel('watcher')->error("STARTUP: .env loading at: ...");
